...
data_train['label'] = [1 if i==1 else 0 for i in data_train.Truth.values.tolist()]
data_valid = pd.read_csv("./dataset/pre-processing/PeriHD_Sentence-merge_internal-test[pos+neg].csv")
data_valid['label'] = [1 if i==1 else 0 for i in data_valid.Truth.values.tolist()]
...
# %%
save_path = "/ssd8/chih/project/yadong/predict_baseline_version02_ACM/other_experiment/dataset/PeriHD_pickle_save_05-17"
save_type_folder = ['mUSE_sample2693_sequence_fullcontext_tf']
...
# save_folder = "USD_sample2693_contextsplit"
    ...
    for idx, (free_text, text_, label_) in tqdm(enumerate(zip(run_data, data_train['處置其他結束（描述）'].to_list(),data_train['label'].to_list()))):
        if "train_USE_internal_pk_data_{}_fixed_re-marked_merge.pkl".format(idx) not in os.listdir(os.path.join(save_path, save_folder, "train")):
            with open(os.path.join(save_path, save_folder, "train", "train_USE_internal_pk_data_{}_fixed_re-marked_merge.pkl".format(idx)), "wb") as f:
                embed_ = session.run(USE_model([free_text]))
...
# save_folder = "USD_sample2693_contextsplit"
    save_folder = save_type_folder[0]
    for idx, (free_text, text_, label_) in tqdm(enumerate(zip(run_data, data_train['處置其他結束（描述）'].to_list(),data_train['label'].to_list()))):
        if "valid_USE_internal_pk_data_{}_fixed_re-marked_merge.pkl".format(idx) not in os.listdir(os.path.join(save_path, save_folder, "valid")):
