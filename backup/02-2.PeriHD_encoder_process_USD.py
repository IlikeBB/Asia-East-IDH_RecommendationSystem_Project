...
data_test = pd.read_csv("./dataset/PeriHD_Sample-2693_Sentence-merge_internal-test.csv")
data_test['label'] = [1 if i=='Y' else 0 for i in data_test.Truth.values.tolist()]
print(data_train.shape, data_test.shape)

# %%
print(len(data_train.label.values)-sum(data_train.label.values), sum(data_train.label.values))
print(len(data_test.label.values)-sum(data_test.label.values), sum(data_test.label.values))
...
#     session.run([tf.global_variables_initializer(), 
#                  tf.tables_initializer()])
#     run_data = data_test['處置其他結束（描述）'].astype(str).to_list()
#     USD_validation_embeddings = session.run(USE_model(run_data))
# save_folder = save_type_folder[0]
# # save_folder = "USD_sample2693_contextsplit"
# for idx, (embed_, text_, label_) in enumerate(zip(USD_validation_embeddings, data_test['處置其他結束（描述）'].to_list(),data_test['label'].to_list())):
...
data_test = pd.read_csv("./dataset/PeriHD_Sample-2693_Sentence-merge_internal-test.csv")
data_test['label'] = [1 if i=='Y' else 0 for i in data_test.Truth.values.tolist()]
merge_df = pd.concat([data_train, data_test])
...
for idx, (text, ans) in enumerate(zip(concat_df.tolist(), ans_list)):
    if idx >=2000:
        print(os.path.join(save_path, save_folder, "train", "train_USD_external_pk_data_{}.pkl".format(idx)))
        ...
        with open(os.path.join(save_path, save_folder, "train", "train_USD_external_pk_data_{}.pkl".format(idx)), "wb") as f:
            ...
            pickle.dump(save_dict, f)
        # break
    # if idx>2000: #500, 1000, 1500, 2000, 2500
    #     break
